 ...
 *
 * Note that deleting a schedule will leave an empty row with a deletion timestamp in the table.
 */
    ...
    try (CloseableIterator<StructuredRow> iterator = triggerStore.scan(triggerField)) {
      while (iterator.hasNext()) {
        ...
        try {
          ...
          StructuredRow scheduleRow = readExistingScheduleRow(scheduleId);
